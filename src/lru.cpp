...
template<class K, class V, std::size_t N>
 inline void LRUCache::LRU<K,V,N>::reset() {
    this->index = 0;
}

template<class K, class V, std::size_t N>
inline bool LRUCache::LRU<K,V,N>::find(const K &key, V &value) {
    unsigned nibbleOffset = NIBBLE_SHIFT_TO_DWORD;
    unsigned idx = this->index;
    LRUCache::LRU<K,V,N>::CacheSlot *currentEntryPtr = (cache - 1);
    while (idx) {
...
template<class K, class V, std::size_t N>
inline void LRUCache::LRU<K,V,N>::insert(const K &key, const V &value) {
    LRUCache::LRU<K,V,N>::CacheSlot *currentEntryPtr = this->cache - 1;
